...
public class PomeloClient extends WebSocketClient {
    ...
    private Map<String, OnDataHandler> onRouteHandlerMap = new HashMap<String, OnDataHandler>();
    ...
    private String handshakeCrypto = null;


    public PomeloClient(URI serverURI, String handshakeCrypto) {
        super(serverURI);
        if(handshakeCrypto.length() > 0)
    ...
    public void listen(String route, OnDataHandler onDataHandler){
        onRouteHandlerMap.put(route, onDataHandler);
    }
    ...
    private void onData(PomeloPackage.Package decode) throws PomeloException, JSONException {
        ...
        if (onDataHandler != null) {
            ...
            onDataHandler = onRouteHandlerMap.get(route);
            if (onDataHandler != null) {
                onDataHandler.onData(message);
            }
            else{
    ...
    private void handshake(PomeloPackage.Package decode) throws JSONException {
        ...
        if(handshakeCrypto != null) {
            body = decrypt(handshakeCrypto, body);
        }
        String resStr = PomeloPackage.strdecode(body);
    ...
    private void initData(JSONObject data) throws JSONException {
        ...
        if (!sys.isNull(HandshakeProvider.HANDSHAKE_SYS_PROTOS_KEY)) {
            ...
            protosVersion = protos.has(HandshakeProvider.HANDSHAKE_SYS_PROTOS_VERSION_KEY) ? protos.getString(HandshakeProvider.HANDSHAKE_SYS_PROTOS_VERSION_KEY) : null;
